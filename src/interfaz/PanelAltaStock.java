...
public class PanelAltaStock extends JPanel {
	...
	public PanelAltaStock(Empresa empresa, Runnable accionVolver, ControladorStock cs) {
		this.accionVolver = accionVolver;
		this.controlador = cs;
		setLayout(new BorderLayout());

		// INSTANCIAMOS EL PANEL MAESTRO
		PanelFormularioProducto formulario = new PanelFormularioProducto( this.controlador, "", // Sin código predefinido

				// 1. ACCIÓN AL GUARDAR (EXITOSO)
				() -> {
					// Al limpiar, creamos un nuevo panel pero LE PASAMOS LA MISMA ACCIÓN
					removeAll();
					add(new PanelAltaStock(empresa, accionVolver,this.controlador));
					revalidate();
					repaint();
				},
